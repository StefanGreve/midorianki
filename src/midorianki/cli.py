...
def build_parser(package_name: str, version: str, description: str) -> ArgumentParser:
    ...
    parser.add_argument(
        "--verbose", default=True, action="store_true", help="increase output verbosity (default: %(default)s)"
    )
    ...
    convert_parser.add_argument(
        ...
        nargs="?",
        default=Path.cwd(),
        help="APKG target directory (default: %(default)s)",
    )
    convert_parser.add_argument(
        "--name", type=str, nargs="?", default=None, help="deck filename and title (default: file stem)"
    )
